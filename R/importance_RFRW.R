importance_RFRW <- function(x, ...)  UseMethod("importance_RFRW")

importance_RFRW.default <- function(x, ...)
    stop("No method implemented for this class of object")

importance_RFRW.RFRW <- function(x, type=NULL, class=NULL, scale=TRUE,
                                    ...) {
    if (!inherits(x, "RFRW"))
        stop("x is not of class RFRW")
