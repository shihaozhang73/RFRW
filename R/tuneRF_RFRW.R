tuneRF_RFRW <- function(x, y, mtryStart=if(is.factor(y)) floor(sqrt(ncol(x))) else
  ...
  classRF <- is.factor(y)
  errorOld <- if (classRF) {
    RFRW(x, y, mtry=mtryStart, ntree=ntreeTry,
  ...
  for (direction in c("left", "right")) {
    ...
    while (Improve >= improve) {
      ...
      errorCur <- if (classRF) {
        RFRW(x, y, mtry=mtryCur, ntree=ntreeTry,
  ...
  if (doBest) 
    res <- RFRW(x, y, mtry=res[which.min(res[,2]), 1], ...)
