rfcv_RFRW <- function(trainx, trainy, cv.fold=5, scale="log", step=0.5,
    ...
    for (i in 1:cv.fold) {
        ## cat(".")
        all.rf <- RFRW(trainx[idx != i, , drop=FALSE],
                               trainy[idx != i],
                               trainx[idx == i, , drop=FALSE],
                               trainy[idx == i],
                               mtry=mtry(p), importance=TRUE, ...)
        cv.pred[[1]][idx == i] <- all.rf$test$predicted
        impvar <- (1:p)[order(importance_RFRW(all.rf, type=1), decreasing=TRUE)]
        for (j in 2:k) {
            imp.idx <- impvar[1:n.var[j]]
            sub.rf <-
                RFRW(trainx[idx != i, imp.idx, drop=FALSE],
                             trainy[idx != i],
            ...
            if (recursive) {
                impvar <-
                    (1:length(imp.idx))[order(importance_RFRW(sub.rf, type=1), decreasing=TRUE)]
      }
      NULL
    }
    NULL
