rfImpute_RFRW <- function(x, ...)
    UseMethod("rfImpute_RFRW")

rfImpute_RFRW.formula <- function(x, data, ..., subset) {
...
rfImpute_RFRW.default <- function(x, y, iter=5, ntree=300, ...) {
    ...
    for (i in 1:iter) {
        prox <- RFRW(xf, y, ntree=ntree, ..., do.trace=ntree,
