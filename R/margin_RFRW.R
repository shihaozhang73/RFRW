margin_RFRW <- function(x, ...) {
    UseMethod("margin_RFRW")
}

margin_RFRW.RFRW <- function(x, ...) {
    ...
    margin_RFRW(x$votes, x$y, ...)
}

margin_RFRW.default <- function(x, observed, ...) {
    ...
    margin_RFRW <- numeric(length(observed))
    for (i in seq_along(observed)) {
        margin_RFRW[i] <- x[i, position[i]] - max(x[i, -position[i]])
    }
    names(margin_RFRW) <- observed
    class(margin_RFRW) <- "margin_RFRW"
    margin_RFRW
}

plot.margin_RFRW <- function(x, sort=TRUE, ...) {
    if (sort) x <- sort(x)
    nF <- factor(names(x))
