...
"RFRW.default" <-
    function(x, y=NULL,  xtest=NULL, ytest=NULL, ntree=500,
             ...
             keep.inbag=FALSE, case_weights=matrix(1,ntree,nrow(x)),...) {
    addclass <- is.null(y)
    ...
    ## case weights for node weighted mean and split criterion
    
    if (nrow(case_weights) != ntree || ncol(case_weights) != n) {
      stop("please supply the case weights with dimension ntree * n")
    }
    if (any(case_weights <= 0)) {
      stop("All case weights must be positive")
    }
    ...
    if (classRF) {
        ...
        if (keep.forest) {
            ...
            # rfout$case_weights <- rfout$case_weights[1:ntree, , drop=FALSE]
        }
        cl <- match.call()
        cl[[1]] <- as.name("RFRW")
        ...
        out <- list(call = cl,
                    type = "regression",
                    # case_weights = rfout$case_weights,
        ...
        ## quantile
        nodesX <- attr(predict(out,x_copy,nodes=TRUE),"nodes")
        ...
        for (tree in 1:ntree){
          ...
          valuesNodes[useNodes,tree] <- y_copy[ind[match(useNodes,shuffledNodes )]]
        }
        
        out[["call"]] <- cl
        out[["valuesNodes"]] <- valuesNodes
    }
