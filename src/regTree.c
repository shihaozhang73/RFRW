...
void regTree(double *x, double *y, int mdim, int nsample, int *lDaughter,
             ...
             double *nodeImpurityDecrease, int *varUsed, double *case_weights) {
  ...
  double label, sumOfSquares, nodeMean, decsplit, ubest, sumOfNodeData, case_weights_sum, *case_weights_sums, currentCaseWeightsSum;

  nodestart = (int *) Calloc(numNodes, int);
  numPointsInEachNode   = (int *) Calloc(numNodes, int);
  // sum of case weights for each node
  case_weights_sums   = (double *) Calloc(numNodes, double);
  ...
  zeroDouble(case_weights_sums, numNodes);
  ...
  for (i = 0; i < nsample; ++i) {
    case_weights_sum += case_weights[rowIndices[i] - 1];
    nodeMean += y[rowIndices[i] - 1] * case_weights[rowIndices[i] - 1];
  }
  nodeMeans[0] = nodeMean/case_weights_sum;
  case_weights_sums[0] = case_weights_sum;

  /* for (i = 0; i < nsample; ++i) {
    sumOfSquares += (y[rowIndices[i] - 1] - nodeMean) * (y[rowIndices[i] - 1] - nodeMean);
  } */ 
  ...
  for (k = 0; k < numNodes - 2; ++k) {
    ...
    currentCaseWeightsSum = case_weights_sums[k];
    sumOfNodeData = currentCaseWeightsSum * nodeMeans[k];
    splitResult = 0;
    decsplit = 0.0;
    ...
    findBestSplit(x, rowIndices, y, mdim, nsample, ndstart, ndend, &msplit,
                  ...
                  currentNodeCount, cat, currentCaseWeightsSum,case_weights);
#ifdef RF_DEBUG
    ...
    for (j = ndstart; j <= leftChildLastDataPoint; ++j) {
      case_weights_sum += case_weights[rowIndices[j] - 1];
      nodeMean += y[rowIndices[j]-1] * case_weights[rowIndices[j] - 1];
    }

    nodeMeans[currentNode+1] = nodeMean/case_weights_sum;
    case_weights_sums[currentNode+1] = case_weights_sum;
    ...
    nodeMeans[currentNode + 2] = nodeMean/case_weights_sum;
    case_weights_sums[currentNode + 2] = case_weights_sum;
...
void findBestSplit(double *x, int *jdex, double *y, int mdim, int nsample,
                   ...
                   double sumnode, int nodeCount, int *cat, double currentCaseWeightsSum, double *case_weights) {
  ...
  double *xt, *ut, *v, *yl, sumcat[MAX_CAT], avcat[MAX_CAT], tavcat[MAX_CAT], valueAtBestSplit, *weights_copy;
  double crit, bestSplitForAllVariables, bestSplitWithinVariable, suml, sumr, d, critParent, weights_l, weights_r;
  ...
  weights_copy = (double *) Calloc(nsample, double);
  ...
  for (i = 0; i < mtry; ++i) 
  {
    ...
    if (numCategoriesForVar == 1) {
      ...
      for (j = ndstart; j <= ndend; ++j) {
        ...
        yl[j] = y[jdex[j] - 1];
        weights_copy[j] = case_weights[jdex[j] - 1];
      }
    } else {
    ...
    critParent = sumnode * sumnode / currentCaseWeightsSum;
    suml = 0.0;
    sumr = sumnode;
    weights_l = 0.0;
    weights_r = currentCaseWeightsSum;
    //npopl = 0;
    //npopr = nodeCount;
    crit = 0.0;
    tieVal = 1;
    ...
    for (j = ndstart; j <= ndend - 1; ++j) {
      d = yl[ncase[j] - 1] * weights_copy[ncase[j] - 1];
      suml += d;
      sumr -= d;
      //npopl++;
      //npopr--;
      weights_l += weights_copy[ncase[j] - 1];
      weights_r -= weights_copy[ncase[j] - 1];

      if (v[j] < v[j+1]) {
        //crit = (suml * suml / npopl) + (sumr * sumr / npopr) - critParent;
        crit = (suml * suml / weights_l) + (sumr * sumr / weights_r) - critParent;
