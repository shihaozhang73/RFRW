...
void regRF(double *x, double *y, int *useweights, double *weights, int *xdim, int *sampsize,
	   ...
	   int *cat, int *maxcat, int *jprint, int *doProx, int *oobprox,
           ...
           int *nout, int *inbag, double *case_weights) {
    /*************************************************************************
   Input:
  ...
  *************************************************************************/
    ...
    double *yb, *xtmp, *xb, *ytr, *ytree, *tgini, *case_weights_tree;
    ...
    // case weights for each tree
    case_weights_tree = (double *) S_alloc(*sampsize, sizeof(double));
    ...
    /* include all the data, instead of the bootstraped data*/
    ...
    for (j = 0; j < *nTree; ++j) {
        ...
        for(n = 0; n < *sampsize; ++n){
			...
			in[k] += 1;
			yb[n] = y[k];
            case_weights_tree[n] = case_weights[n + j * nsample];
			for(m = 0; m < mdim; ++m) {
		...
		errb = 0.0;
		ooberr = 0.0;
		jout = 1; /* jout is the number of cases that has been OOB so far (set to 1 instead of 0 in case of denominator is 0)*/
		nOOB = 1; /* nOOB is the number of OOB samples for this tree (set to 1 instead of 0 in case of denominator is 0)*/
		for (n = 0; n < nsample; ++n) {
			if (in[n] == 0) {
				nout[n]++;
                nOOB++;
